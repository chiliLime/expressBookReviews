...
public_users.post("/register", (req,res) => {
  ...
  if (username && password) {
    if (!isValid(username)) {
      ...
      return res.status(200).json({message:`New user is ${username} registered.`});
    }
    else {
      return res.status(400).json({message:`The username ${username} is already registered.`});
    }
  }
  else {
...
function getByISBN(isbn) {
  return new Promise((resolve, reject) => {
    ...
    if (books[isbnNumber]) {
      ...
      reject({status:404, message:`ISBN ${isbn} was not found`});
    }
  })
}
